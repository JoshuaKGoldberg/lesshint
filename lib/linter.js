...
var runChecks = function (ast, fullPath, lines, linesRaw, config) {
    ...
    var checkNode = function (node) {
        linters.forEach(function (linter) {
            ...
            if (lint) {
                ...
                lint.forEach(function (complaint) {
                    var line = complaint.line || node.source.start.line;
                    ...
                    complaint.position = complaint.column
                        + linesRaw.slice(0, complaint.line - 1).join('').length
                        - 1;

                    if (linter.suggestFix) {
                        complaint.suggestedFix = linter.suggestFix(complaint, options, node);
                    }
...
exports.lint = function (input, fullPath, config) {
    ...
    lines = input.split(/\r\n|\n\r|\n|\r/g); // Normalize line endings. See #40
    linesRaw = input.match(/[^\n]+(?:\r?\n|$)/g); // Same as input, but with line endings
    ...
    try {
        ...
        results = sortBy(results, 'position');

        return results;
    } catch (e) {
        // Attach the offending file to the exception, so we know which file is the culprit
        e.lesshintFile = fullPath;
